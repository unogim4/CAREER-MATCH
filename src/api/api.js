...
// 추후 실제 API 연동 시 사용할 기본 설정
const api = axios.create({
  baseURL: 'https://api.example.com', // 사람인 API URL로 변경 예정
  headers: {
    'Content-Type': 'application/json',
  }
});

// 더미 데이터 - 추후 실제 API로 대체
const frontendJobs = [
  {
    title: '시니어 프론트엔드 개발자',
    company: '테크스타트(주)',
...
// API 요청 함수들 (현재는 더미 데이터 반환)
export const getJobRecommendations = (devType, skills) => {
...
export const getSkillAnalysis = (devType, skills, career) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      if (devType === 'frontend') {
        resolve(frontendAnalysis);
      } else {
        resolve(backendAnalysis);
      }
    }, 1000);
  });
};

export const getImprovement = (devType, skills) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      if (devType === 'frontend') {
        resolve(frontendImprovement);
      } else {
        resolve(backendImprovement);
      }
    }, 1000);
  });
};
